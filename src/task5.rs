...
Преобразование вектора байтов в строку: Во втором пропуске используется String::from_utf8(v)
для преобразования вектора байтов в строку. Этот метод возвращает Result, поэтому необходимо обработать возможную ошибку с помощью expect.
*/